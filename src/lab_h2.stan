...
parameters {
	real<lower=0.0> H0; //initial conditions are parameters
	real<lower=0.0> I0;
	real<lower=0.0> P0;
...
model {	
	...
	H0 ~ normal(H[1], 0.5*H[1]);
	I0 ~ normal(I[1], 0.5*I[1]);
	P0 ~ normal(P[1], 0.5*P[1]);

	//--LIKELIHOOD-----------------------------------//
	H ~ normal(x[time_points,1], sigma_H);
